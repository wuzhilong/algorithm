 ...
 *  Tested 500000 nodes, it worked well for insert/remove.
...
public:
    ...
    Node* insert( Node* p,  Node* child )
    {
        if(!root && !p)
        {
            ...
            MY_DEBUG(cout<<" insert at root: key="<<child->key <<endl);
            return root;
        }
        ...
        if( p->key == child->key )
        {
            MY_DEBUG(cout<<"key already exists, key ="<<child->key <<endl);
            return p;
        }
        else if( p->key > child->key )
        {
            if( !(p->left) )
            {
                ...
                if( !(p->right)) //p has only this child now.
                    p->h++;
                child->parent = p;
               MY_DEBUG(cout<<" insert under left of parent(" <<p->key<< "), key="<< child->key  <<endl);
            }
            else
            {
                insert( p->left, child );
        ...
        else //if( p->key < child->key )
        {
            if( !(p->right ))
            {
                ...
                if( !(p->left)) //p has only this child now.
                    p->h++;
                child->parent = p;
                MY_DEBUG( cout<<" insert under right of parent("<< p->key <<"), key="<< child->key  <<endl);
            }
            else
            {
    ...
    Node* remove( int key )
    {
        if(!root )
        {
            MY_DEBUG(cout<<" remove NULL tree or NULL node!"<< endl);
            return root;
        }
        Node* ptmp = search( root, key );
        if( !ptmp)
        {
            MY_DEBUG(cout<<" key dones not exist in the tree, remove NULL."<<endl); 
            return NULL;
        }
        ptmp = remove_helper( ptmp, key );
        return ptmp; 
    }
...
private:
    ...
    Node* remove_helper(Node* n, int key )
    {
        ...
        if( !ptmp)
        {
            MY_DEBUG(cout<<" remove_helper, ret NULL."<<endl); 
            return NULL;
        }
        Node* pbalance_start = ptmp->parent; //default is from the delete Node's parent to do balance.
        //1. if it is a leaf. simply remove it, and set its parent to NULL.
        if( !ptmp->left && !ptmp->right )
        {
            MY_DEBUG(cout<<"remove leaf:"<< *ptmp <<endl);
            //replace_node( ptmp, NULL );
            if( !(ptmp->parent) )
                root = NULL;
            else
            {
                if( ptmp->parent->left == ptmp )  
                    ptmp->parent->left = NULL;
                else 
                    ptmp->parent->right = NULL;
                MY_DEBUG( cout<< "replace_node: set old to NULL.return!"<<endl);
            }
        }
        //2. if it has only a right child. replace this ptmp with the its right child.
        else if( !ptmp->left && ptmp->right )
        {
            MY_DEBUG(cout<<"remove node:"<< *ptmp<< "replace with Right child:"<< *(ptmp->right) <<endl);
            //replace_node( ptmp, ptmp->right );
            ptmp->right->parent = ptmp->parent; 
            if( !(ptmp->parent) )
                root = ptmp->right;
            else
            {
                if( ptmp->parent->left == ptmp )  
                    ptmp->parent->left = ptmp->right;
                else 
                    ptmp->parent->right = ptmp->right;
            }
        }
        //3. if it has only a left child. replace this ptmp with its left child .
        else if( ptmp->left && !ptmp->right )
        {
            MY_DEBUG(cout<<"remove node:"<< *ptmp<< "replace with Left child:"<< *(ptmp->left) <<endl);
            //replace_node( ptmp, ptmp->left );
            ptmp->left->parent = ptmp->parent; 
            if( !(ptmp->parent) )
                root = ptmp->left;
            else
            {
                if( ptmp->parent->left == ptmp )  
                    ptmp->parent->left = ptmp->left;
                else 
                    ptmp->parent->right = ptmp->left;
            }
        }
    /*
           p                                     p
       ...
       */
        ...
        {
            MY_DEBUG(cout<<"remove node:"<< *ptmp<< "has both child, L:"<< *(ptmp->left)<< " R:"<< *(ptmp->right) <<endl);
            ...
            ptmp->key = pmin->key;
            ptmp = pmin;
            pbalance_start = pmin->parent;  //here balance start from successor parent(r)
            ptmp = remove_helper( ptmp, pmin->key ); 
            /*
            if( pmin  != ptmp->right )
            {
                MY_DEBUG(cout<<"remove node replace with successor!= right node:"<< *ptmp<< " Succ:"<< *(pmin)<< " R:"<< *(ptmp->right) <<endl);
                pbalance_start = pmin->parent;  //here balance start from successor parent(r)
                //replace_node( pmin, pmin->right ); // use m to replace x
                pmin->right->parent = pmin->parent; 
                assert( pmin->parent->left == pmin ); 
                pmin->parent->left = pmin->right;
            }
            else 
            {
                MY_DEBUG(cout<<"remove node replace with successor==right node:"<< *ptmp<< *(ptmp->right) <<endl);
                pbalance_start = pmin;  //4.2 balance start from this successor. 
            }
            //4.2 its successor is its right child. same as case2.
            replace_node( ptmp, pmin );
            */
        }

        MY_DEBUG(cout<< "!!!!after delete and before balance_tree" << endl);
        printAVLTree( this );
        while( pbalance_start )  // go upward till the tree's root, to do balance.
        {
            pbalance_start = balance_tree( pbalance_start );
            if( pbalance_start ) 
                pbalance_start = pbalance_start->parent;
        }

        MY_DEBUG(cout<< "!!!after delete and re-balance_tree" << endl);
        //printAVLTree( this );
        return ptmp;
    };
    ...
    Node* balance_tree( Node* p)
    {
        if( !p )
            return NULL;
        MY_DEBUG(cout<< "balance_tree, Node:"<<*p);
        ...
        {
            //since balance >2, p->right must be valid.
            assert( p->right );
    ...
    Node* replace_node( Node* old_node, Node* new_node )
    {
        ...
        MY_DEBUG(cout<< "before replace_node: old:"<< *old_node <<"new:"<<*new_node<<endl);

        if( old_node->parent == new_node) // new_node is parent,only replace children 
        {
          MY_DEBUG(cout<<"replace_node, o->p = n " <<endl);
        ...
        {
           MY_DEBUG( cout<<"replace_node, o->r = n " <<endl);
            new_node->left = old_node->left;
            if( new_node->left )
                new_node->left->parent = new_node;
        ...
        {
            MY_DEBUG(cout<<"replace_node, o->l = n " <<endl);
            new_node->right = old_node->right;
            if( new_node->right )
                new_node->right->parent = new_node;
        ...
        else  // normal case, not relationship between two nodes.
        {
            ...
            MY_DEBUG(cout<<"replace_node, normal case, n != o" <<endl);
            new_node->parent = old_node->parent;
            new_node->right = old_node->right;
            new_node->left = old_node->left;
            if( new_node->right )
        ...
        MY_DEBUG(cout<< "after replace_node: old:"<< *old_node <<"new:"<<*new_node<<endl);
        return old_node;
    };
...
/*
  ...
  /*  for( auto x:vec1 )
    ...
    */
    ...
    //for(int j = 0; j< size; )
    {
        int j = (int) ((double)rand()/(double)RAND_MAX * (size*1.2) );
        if( j >= vec.size()) 
            j = vec.size() -1;
        ...
        d++;
        cout<<"\n\n after delete, before verify!!! running number: "<< d <<","<<r<<" keys being removed" <<" ,deleted key="<< k <<endl;
        //printAVLTree(&avltree);
        validate_tree(&avltree, avltree.root);
